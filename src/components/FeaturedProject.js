...
const FeaturedProject = ({data}) => {
    ...
    return (
        <div key={data.id} className="container min-h-max group relative grid grid-cols-12 max-sm:grid-cols-1 gap-2 mb-10 mt-10">
            <div className="relative max-sm:p-2 p-5 sm:col-start-1 sm:col-end-7 row-start-1 row-end-[-1] z-20 max-sm:col-start-1 max-sm:col-end-[-1]">
                <h5 className="text-secondary-500 text-sm mb-1 drop-shadow-lg">Featured Project</h5>
                ...
                <ul className="flex flex-row flex-wrap">
                    {
                        data.tech.map( (t,index) => {
                            return <li className="p-1 text-sm drop-shadow-md text-secondary-500 hover:text-accent-400" key={index}>{t}</li>
                        })
                    }
                </ul>
            ...
            <a className="relative group image-tint z-10 bg-cyan-400  max-h-[300px] rounded-xl  overflow-hidden sm:col-start-4 sm:col-end-[-1] max-sm:col-start-1 max-sm:col-end-[-1] row-start-1 row-end-[-1]" href={data.external} target="_blank">
